...
class MonoWaveUp:
    ...
    def next_high(self, idx_start: int = 0, prev_high: float = 0):
        """
        ...
        :return:
        """
        high = self.df.loc[idx_start]['High']
        high_idx = idx_start
        ...
        for idx, row in self.df.loc[idx_start+1:].iterrows():
            act_high = row['High']
    ...
    def find_end(self, idx_start: int):
        ...
        :return:
        """
        high, high_idx = self.next_high(idx_start)
        if high is None:
            return None, None

        for _ in range(self.skip_n):

            act_high, act_high_idx = self.next_high(high_idx, high)
            if act_high > high:
                high = act_high
...
class MonoWaveDown:
    ...
    def next_min(self, idx_start: int = 0, prev_low: float = 0):
        low = self.df.loc[idx_start]['Low']
        low_idx = idx_start
    ...
    def find_end(self, idx_start: int):
        ...
        :return:
        """
        low, low_idx = self.next_min(idx_start)
        if low is None:
            return None, None

        for _ in range(self.skip_n):

            act_low, act_low_idx = self.next_min(low_idx, low)
            if act_low is None:
                return None, None
...
class WaveOptions:
    def __init__(self, i:int, j: int, k: int, l: int, m:int):
        self.i = i
        self.j = j
        self.k = k
        self.l = l
